...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    var uploadfsDefaultSettings = {
      backend: "local",
      uploadsPath: self.apos.rootDir + "/private/uploads",
      uploadsUrl:
        (self.apos.baseUrl || "") + self.apos.prefix + "/private/uploads",
    ...
    self.apostropheDestroy = function(callback) {
      if (!self.uploadfs.destroy) {
        self.apos.utils.warn(
          "uploadfs dependency is old, cannot clean up resources i.e. timers that will prevent exit"
        );
        return callback(null);
      }
    ...
    self.enableCollection = function(callback) {
      return async.series(
        [collection, indexDocIds, indexTrashDocIds],
        callback
      );
      function collection(callback) {
        return self.apos.db.collection("aposAttachments", function(
          err,
          collection
        ) {
          self.db = collection;
...
// put what is below in pieces routes file inside the insertViaUpload method.

// var type = req.files.files.type;
