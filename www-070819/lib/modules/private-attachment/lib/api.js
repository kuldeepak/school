...
module.exports = function(self, options) {
  ...
  self.insert = function(req, file, options, callback) {
    ...
    function body(callback) {
      ...
      if (!group) {
        ...
        return callback(
          "File extension not accepted. Acceptable extensions: " +
            accepted.join(",")
        );
      }
      ...
      var info = {
        ...
        group: group.name,
        createdAt: new Date(),
        name: self.apos.utils.slugify(
          path.basename(file.name, path.extname(file.name))
        ),
        title: self.apos.utils.sortify(
          path.basename(file.name, path.extname(file.name))
        ),
        extension: extension,
        type: "attachment",
      ...
      function permissions(callback) {
        ...
        return callback(
          self.apos.permissions.can(req, "edit-attachment") ? null : "forbidden"
        );
      }
      ...
      function upload(callback) {
        if (image) {
          ...
          return self.uploadfs.copyImageIn(
            file.path,
          ...
          // be reasonably safe)
          return self.uploadfs.copyIn(
            file.path,
            "/attachments/" + info._id + "-" + info.name + "." + info.extension,
            callback
          );
        }
      }
      ...
      return async.series(
        [permissions, length, md5, upload, remember],
        function(err) {
  ...
  self.clone = function(req, source, callback) {
    var originalFile =
      "/attachments/" + source._id + "-" + source.name + "." + source.extension;
    var tempFile =
      self.uploadfs.getTempPath() +
      "/" +
      self.apos.utils.generateId() +
  ...
  self.url = function(attachment, options) {
    ...
    var path = "/private/attachments/" + attachment._id + "-" + attachment.name;
  ...
  self.hasFocalPoint = function(attachment) {
    ...
    return (
      attachment._focalPoint && typeof attachment._focalPoint.x === "number"
    );
  };
  ...
  self.addTypeMigration = function() {
    self.apos.migrations.add(
      self.__meta.name + ".addType",
      function(callback) {
        var needed;
        ...
        function checkNeeded(callback) {
          return self.db.findOne({ type: { $exists: 0 } }, function(
            err,
            found
          ) {
            if (err) {
  ...
  self.addDocReferencesMigration = function() {
    self.apos.migrations.add(
      self.__meta.name + ".docReferences",
      function(callback) {
        var needed;
        var attachmentUpdates = {};

        return async.series(
          [checkNeeded, docs, attachments, self.updatePermissions],
          callback
        );

        function checkNeeded(callback) {
          return self.db.findOne({ docIds: { $exists: 0 } }, function(
            err,
            found
          ) {
            if (err) {
        ...
        function attachments(callback) {
          ...
          return async.series(
            [applyAttachmentUpdates, docIds, trashDocIds],
            callback
          );
          function applyAttachmentUpdates(callback) {
            return async.eachLimit(
              _.keys(attachmentUpdates).slice(),
              5,
  ...
  self.addFixPermissionsMigration = function() {
    self.apos.migrations.add(
      self.__meta.name + ".fixPermissions",
      function(callback) {
        return self.apos.migrations.each(
          self.db,
          { group: "images", trash: true },
          5,
          ...
          function(err) {
            ...
            self.apos.utils.warn(
              "^^^ WARNINGS ABOVE ARE OK. This migration fixes a scenario in which SOME permissions were not set."
            );
            return callback(null);
          }
        );
  ...
  self.updateDocReferences = function(doc, callback) {
    ...
    return async.series([updateCounts, self.updatePermissions], callback);
  ...
  self.updatePermissions = function(callback) {
    return async.series([hide, show], callback);
  ...
  self.applyPermissions = function(attachment, trash, callback) {
    ...
    return async.series([original, crops], callback);
    ...
    function cropOne(crop, callback) {
      return async.eachSeries(
        self.imageSizes.concat([{ name: "original" }]),
        function(size, callback) {
          if (size.name === self.sizeAvailableInTrash) {
            // This size is always kept accessible for preview
            // in the media library
