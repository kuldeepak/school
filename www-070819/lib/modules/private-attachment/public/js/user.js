apos.define("private-attachment", {
  ...
  construct: function(self, options) {
    ...
    self.populate = function(object, name, $field, $el, field, callback) {
      ...
      $input.fileupload({
        ...
        always: function(e, data) {
          ...
          var groups =
            field.fileGroups || (field.fileGroup && [field.fileGroup]);
          if (groups) {
            if (!_.contains(groups, file.group)) {
              extensions = [];
              _.each(groups, function(group) {
                var groupInfo = _.find(self.options.fileGroups, {
                  name: group
                });
                if (!groupInfo) {
                  return;
              ...
              apos.notify(
                "The file must be of one of the following types: %s",
                extensions.join(", "),
                { type: "error" }
              );
              return;
          ...
          extensions =
            field.extensions || (field.extension && [field.extension]);
          if (extensions) {
    ...
    self.updateExisting = function($fieldset, info, field) {
      ...
      $existing.alterClass(
        "apos-extension-*",
        "apos-extension-" + info.extension
      );
      var $preview = $existing.find("[data-preview]");
      if (info.group === "images") {
        $preview.attr(
          "src",
          self.url(info, { size: "one-half", crop: info.crop })
        );
