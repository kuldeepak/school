...
module.exports = function(self, options) {
  self.createRoutes = function() {
    ...
    self.route(
      "post",
      "retrieve",
      self.requirePieceEditorView,
      self.routes.retrieve
    );
    ...
    self.route(
      ...
      "manager-modal",
      self.requireEditor,
      self.routes.managerModal
    );
    self.route(
      "post",
      "chooser-modal",
      self.requireEditor,
      self.routes.chooserModal
    );
    self.route(
      "post",
      "editor-modal",
      self.requireEditor,
      self.routes.editorModal
    );
    self.route(
      "post",
      "create-modal",
      self.requireEditor,
      self.routes.createModal
    );
    ...
    self.route(
      "post",
      "insert-via-upload",
      self.apos.attachments.middleware.canUpload,
      self.apos.middleware.files,
      self.routes.insertViaUpload
    );
  };
    ...
    return async.series(
      ...
      function(err) {
        ...
        if (req.body.format === "managePage") {
          ...
          var viewTemplate =
            "manage" + self.apos.utils.capitalizeFirst(view) + "View";
          results = {
          ...
          results = {
            ids: results.ids,
            label: req.__(
              "Select all %s item(s) that match this search",
              results.ids.length
            )
          };
        }
  ...
  self.routes.publish = function(req, res) {
    return self.batchSimpleRoute(req, "publish", function(
      req,
      piece,
      data,
      callback
  ...
  self.routes.unpublish = function(req, res) {
    return self.batchSimpleRoute(req, "unpublish", function(
      req,
      piece,
      data,
      callback
  ...
  self.routes.tag = function(req, res) {
    return self.batchSimpleRoute(req, "tag", function(
      req,
      piece,
      data,
      callback
  ...
  self.routes.untag = function(req, res) {
    return self.batchSimpleRoute(req, "untag", function(
      req,
      piece,
      data,
      callback
  ...
  self.routes.editorModal = function(req, res) {
    ...
    return res.send(
      self.render(req, "editorModal", { options: self.options, schema: schema })
    );
  };
  ...
  self.routes.createModal = function(req, res) {
    ...
    self.apos.schemas.bless(req, schema);
    return res.send(
      self.render(req, "createModal", { options: self.options, schema: schema })
    );
  };
  ...
  self.routes.trash = function(req, res) {
    ...
    if (req.body.job) {
      // New way
      return self.apos.modules["apostrophe-jobs"].run(
        req,
        function(req, id, callback) {
  ...
  self.routes.insertViaUpload = function(req, res) {
    ...
    if (self.options.privateUpload) {
      type = "private-attachment";
    } else {
      type = "attachment";
    }

    var field = _.find(self.schema, { type: type });
    if (!field) {
      self.apos.utils.error(
        "The pieces module " +
          self.options.__meta.name +
          " has insertViaUpload turned on but the schema does not contain a field of type attachment."
      );
      res.statusCode = 404;
      return res.send("notfound");
    }
    ...
    var attachment = null;
    var piece;
    return async.series([acceptFile, insertPiece, filter], respond);

    function acceptFile(callback) {
      if (self.options.privateUpload) {
        return self.apos.modules["private-attachment"].insert(
          req,
          file,
        ...
        return self.apos.attachments.insert(req, file, function(
          err,
          _attachment
        ) {
          if (err) {
